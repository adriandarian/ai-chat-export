...
export const SelectionPanel = ({
  ...
  return (
    <div className={`fixed bottom-4 right-4 z-[10001] shadow-2xl rounded-lg w-80 overflow-visible flex flex-col max-h-[500px] pointer-events-auto font-sans transition-colors ${
      isDark 
        ? 'bg-slate-800 border border-slate-700' 
        : 'bg-white border border-gray-200'
    }`}>
      {/* Header */}
      <div className={`p-3 border-b flex justify-between items-center rounded-t-lg ${
        isDark 
      ...
      {/* Footer */}
      <div className={`p-3 border-t flex flex-col gap-2 rounded-b-lg relative ${
        isDark 
          ? 'bg-slate-900 border-slate-700' 
          : 'bg-gray-50 border-gray-200'
      }`}>
        {/* Loading Overlay */}
        {isExporting && (
          <div className={`absolute inset-0 backdrop-blur-sm flex flex-col items-center justify-center z-10 rounded-b-lg ${
